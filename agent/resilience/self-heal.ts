...
export function scoreStringSimilarity(query: string, candidate: string): number {
...
export async function healSelector(page: Page, query: string): Promise<HealedSelector | null> {
  const candidates = (await page.evaluate(() => {
    const elements = Array.from(
      document.querySelectorAll("button, a, input, textarea, [role], [contenteditable='true']")
    ).slice(0, 250);
  ...
  let best: { score: number; selector: HealedSelector | null } = { score: 0, selector: null };
