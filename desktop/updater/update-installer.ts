...
/** Staging dirs older than this are swept at startup. */
...
export class UpdateInstaller {
  ...
  /**
   * Remove staging directories left by earlier updates. The post-quit script
   * frees the archive it downloaded, but it cannot delete the directory it is
   * running from, and a run that dies before that point leaves everything.
   * Called at startup, so the version that lands is the one that tidies up
   * after the version before it — including builds that predate #172.
   */
  async sweepStale(maxAgeMs = DEFAULT_STAGING_TTL_MS): Promise<number> {
    let removed = 0;
    let entries: string[];
    try {
      entries = await fs.readdir(this.scratchRoot);
    } catch {
      return 0; // nothing has ever been staged
    }
    const cutoff = Date.now() - maxAgeMs;
    for (const entry of entries) {
      const dir = path.join(this.scratchRoot, entry);
      try {
        ...
        // A directory we cannot stat or remove is not worth failing startup.
      }
    }
    return removed;
  }
...
export function buildSwapScript(): string {
  ...
  //   $5 LOG_PATH      (inside STAGING_DIR)
  //   $6 STAGING_DIR   (e.g. /tmp/marshal-update/v0.1.5-XXXX)
  return `#!/bin/bash
...
# Drop the downloaded archive and the now-empty extract dir. Every successful
# update used to leave its 131 MB zip behind forever — nothing else cleaned up
# after this script, and three updates measured 407 MB of dead weight (#172).
#
# The script itself lives inside STAGING_DIR, so the directory is NOT removed
# here: deleting it would pull the file out from under the bash that is still
# reading it. The script and the log are a few KB and stay as a record; the
# next launch sweeps them.
if [ -n "$STAGING_DIR" ] && [ -d "$STAGING_DIR" ]; then
  rm -f "$STAGING_DIR/marshal.zip"
  rm -rf "$STAGING_DIR/extracted"
  echo "[post-quit-installer] freed the downloaded archive"
fi
