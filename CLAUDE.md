...
- E2E у реальному Electron — окремо від vitest: `npm run test:e2e` (smoke),
  `npm run test:e2e:capture` (редактор захоплення), `npm run test:e2e:alternatives`
...
- **Live captions (`desktop/captions/`) — оверлей, якого не видно у screen share.** Це не
  ...
  **Живий рядок під час мовлення (#203):** раніше на оверлеї не було **нічого**, доки фраза
  ...
  `MARSHAL_CAPTIONS_PARTIALS`: не задано — увімкнено для віддаленого STT і вимкнено для
  `whisper-cpp` (кожен прохід там — CPU на машині з дзвінком); `1`/`0` — примусово.
